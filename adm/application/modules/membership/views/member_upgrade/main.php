...
function select_lookup_member()
{
	...
	showProgres();
	$.post(site_url+"membership/upgrade/lookup_member/"
		...
		,function(result) {
			...
			}else
			{
				...
				$('#category').val(result['member']['fidMemberCategory']+' - '+result['membercat']['Description']);
				$('#category_upgrade').focus();
				$('#category_upgrade').select();
			}
			hideProgres();
		}					
		,"json"
...
{
	showProgres();
	$.post(site_url+"membership/upgrade/update_member_card/"
		,{t_member_code:$('#member_code').val()
		,t_barcode:$('#barcode').val()
		,t_remark:$('#remark').val()
		,t_member_category:$('#category').val()
		,t_category_upgrade:$('#category_upgrade').val()}
		,function(result) {
			if(result['error'])
			{
				hideProgres();
                ...
                <div class="card-content collapse show">
                  <div class="card-body">		
						...
						<div class="form-group row">
							  ...
							  <div class="col-md-5">
								<div class="input-group">
									<div class="input-group-prepend">
										<span class="input-group-text">
										<i class="la la-share-alt"></i></span>
									</div>
									<input readonly type="text" id="category" class="form-control" placeholder="Member Category" name="t_member_category">
								</div> 
							  </div>
						</div>
						<div class="form-group row" >
						  <label class="col-md-3 label-control" for="phone_number">Upgrade Category</label>
						  <div class="col-md-5">
							<div class="form-group">							
								<select class="form-control select2"  style="width: 100%;" id="category_upgrade" name="t_category_upgrade">
										<?php
											foreach($membercats->result_array() as $row)
										{
										?>
									<option value="<?= $row['MemberCategory']?>"><?= $row['MemberCategory'].' : '.$row['Description']?></option>
										<?php } ?>
								</select>							
							</div>
						  </div>
					...
					<div class="form-group row" id="btn_change_card" style="display:none">
					  ...
					  <div class="col-md-5">
						<div class="input-group">
							<button type="button" class="btn btn-success round btn-min-width mr-1 mb-1 loading-button" onClick="update_member_card()"> <i class="la la-check-circle-o"></i> Upgrade Member</button>
						</div>
					  </div>
					</div> 
                  </div>
