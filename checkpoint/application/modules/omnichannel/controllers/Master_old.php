...
class Master extends MY_Controller {
	...
	function page($pg,$report_type=1)
	{
		...
		$list_all=$this->product_parent_model->get_list_old();
		...
		$list=$this->product_parent_model->get_list_old();	
		...
		if($report_type==2){
		/** REPORT TYPE EXCEL **/
			$filename = 'product_management_'.$mitra.'';
			...
			$objPHPExcel->getActiveSheet()->setCellValue('C3', 'Store Description' );
			$objPHPExcel->getActiveSheet()->setCellValue('D3', 'SKU' );
			$objPHPExcel->getActiveSheet()->setCellValue('E3', 'Product Description' );
			$objPHPExcel->getActiveSheet()->setCellValue('F3', 'Qty' );
			$objPHPExcel->getActiveSheet()->setCellValue('G3', 'Disc' );
			$objPHPExcel->getActiveSheet()->setCellValue('H3', 'GrossSales' );
			$objPHPExcel->getActiveSheet()->setCellValue('I3', 'RetailSales' );
			$objPHPExcel->getActiveSheet()->setCellValue('J3', 'MClass' );
			$objPHPExcel->getActiveSheet()->setCellValue('K3', 'Category' );
			$objPHPExcel->getActiveSheet()->setCellValue('L3', 'Brand' );
			$objPHPExcel->getActiveSheet()->setCellValue('M3', 'Delivery' );
			
			$objPHPExcel->getActiveSheet()->setCellValue('A1', 'Mitra' );
			$objPHPExcel->getActiveSheet()->setCellValue('A2', 'Periode' );
			$objPHPExcel->getActiveSheet()->setCellValue('B1', $mitra."-".$mitra_name );
			$objPHPExcel->getActiveSheet()->setCellValue('B2', $tahun." K".$kuartal);
			...
			$no=4;
			$number = 1 ;
			foreach($list_all->result() as $row)
			{
				$objPHPExcel->getActiveSheet()->setCellValue('A'.$no,  $number);
				$objPHPExcel->getActiveSheet()->setCellValue('B'.$no,$row->StoreCode);
				$objPHPExcel->getActiveSheet()->setCellValue('C'.$no,$row->StoreDescription);
				$objPHPExcel->getActiveSheet()->setCellValue('D'.$no, '="'.$row->SKU.'"');
				$objPHPExcel->getActiveSheet()->setCellValue('E'.$no,$row->ProdDescription);
				$objPHPExcel->getActiveSheet()->setCellValue('F'.$no,$row->QtyEPO);
				$objPHPExcel->getActiveSheet()->setCellValue('G'.$no,$row->Disc);
				$objPHPExcel->getActiveSheet()->setCellValue('H'.$no,$row->GrossSales);
				$objPHPExcel->getActiveSheet()->setCellValue('I'.$no,$row->RetailSales);
				$objPHPExcel->getActiveSheet()->setCellValue('J'.$no,$row->MClass);
				$objPHPExcel->getActiveSheet()->setCellValue('K'.$no,$row->Category);
				$objPHPExcel->getActiveSheet()->setCellValue('L'.$no,$row->Brand);
				$objPHPExcel->getActiveSheet()->setCellValue('M'.$no,'Delivery '.$row->PreCat.' - '.$row->DeliveryName);
				$objPHPExcel->getActiveSheet()->getStyle('D'.$no)->getNumberFormat()->setFormatCode('@');
				$number++;
				$no++;
			...
			header('Content-Disposition: attachment;filename=rekap_epo_'.$mitra.'_'.gmdate("ymd_His", time()+60*60*7).'.xlsx');
	...
	$uploadtype = '';
        ...
        if (move_uploaded_file($filedata['tmp_name'], $upload_image)) {
            ...
            $file['ColorCode']     	= $product['fidPrductColor'];
            $file['fidProductSize'] = $product['fidProductSize'];
