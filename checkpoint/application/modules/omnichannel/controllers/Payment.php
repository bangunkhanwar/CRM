...
class Payment extends MY_Controller {
	...
	function page($pg)
	{
		...
		// binding data
		$this->bank_transaction_model->set_limit($limit);
		$this->bank_transaction_model->set_offset($limit * ($pg - 1));
		$this->bank_transaction_model->set_order(array($order_by_field=>$order_by_key));
		// filtering data
		$where = array();
		...
		$key['TransNum']	= $search;
		$list = $this->bank_transaction_model->get_list();
		echo $this->db->last_query();
		// die;
		/*
		*/
		$page = array();
		$page['limit'] 		= $limit;
		$page['count_row'] 	= $this->bank_transaction_model->count();
		$page['current'] 	= $pg;
		...
		$data = array('list' 	=> $list
			...
			,'content' 			=> 'omnichannel/payment/list'
