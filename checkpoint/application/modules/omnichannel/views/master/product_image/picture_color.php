	...
	<div class="row">
		...
		foreach($file_list->result_array() as $row){
			if($row['fileExt'] == 'png' || $row['fileExt'] == 'jpg'|| $row['fileExt'] == 'jpeg'|| $row['fileExt'] == 'gif'){
				$img = '/files/'.$id_parent.'/color/thumb_80/'.$row['fileServerName'];
				$imgOriginal = '/files/'.$id_parent.'/color/original/'.$row['fileServerName'];
			}
		?>
		...
		<div class="card" style = "position: relative;margin-left: -2%;margin-top: -1%;background-color: white;height: 1%;">
				<a style = "color:grey;" href="javascript:void(0)" onClick="removeImageColor('<?=$row['idFileUpload']?>')">
