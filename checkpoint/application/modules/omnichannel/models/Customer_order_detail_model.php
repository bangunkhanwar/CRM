...
class Customer_order_detail_model extends Base_Model {
	...
	function __construct() {
		...
		$this->set_pk('TransNum','StoreCode','RecNum');
		$this->set_log(false);
	}
	...
	function count()
	{
		...
		$this->db->join('public.msProduct msp','tbl.SKU= msp.SKU','left');
	...
	function get_list()
	{
		$this->db->select('tbl.*,mss.Description "StoreName", mts.Description "StatusTrx", mts.StatusColor, mts.SystemRemark,msp.Description');
		$this->db->join('omnichannel.msTransStatus mts','tbl.TransStatus=mts.TransStatus','left');
		$this->db->join('public.msStore mss','tbl.StoreCode=mss.StoreCode','left');
		$this->db->join('public.msProduct msp','tbl.SKU= msp.SKU','left');
	...
	function count_cust_order_detail_sales()
	{
	...
	function get_cust_order_detail_sales()
	{
		$this->db->select('tbl."StoreCode", mss."Description", SUM("GrossSalesTot") "GrossSalesTot", SUM("DiscountTot") "DiscountTot", SUM("RetailSalesTot") "RetailSalesTot"', FALSE);
		$this->db->join('public.msStore mss','tbl.StoreCode=mss.StoreCode','left');
