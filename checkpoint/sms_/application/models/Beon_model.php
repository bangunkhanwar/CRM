...
class Beon_model extends CI_Model 
{
	public $table = '';
	public $schema = '';
	public $pk_field = '';
	...
	{
        parent::__construct();
        $this->db2 = $this->load->database('beon', TRUE);  
    }
	...
	function count() 
	{
		...
		$this->db2->or_like($this->like);
		
		$query = $this->db2->get($this->table.' t1');
	...
	function get($where) 
	{
		...
		{
			$this->db2->where($this->pk_field, $where?:0);
		}
		/*
		if ($this->is_trap)
		{
			$this->db2->select('lg.id_log');
			$sql_log = "(SELECT max(tr.id_log) as id_log,tr.fid_data
							FROM tr_log tr 
							WHERE tr.table_name = '".$this->table."'
							GROUP BY tr.fid_data) lg";
			$this->db2->join($sql_log,'tbl.'.$this->pk_field.' = lg.fid_data','left');
		}
		*/
		// get array data
		$this->db2->select('tbl.*');
		$this->db2->order_by($this->pk_field);
		$query = $this->db2->get($this->schema.'.'.$this->table.' tbl',1,0);
		if ( $query->num_rows() == 1)
		{
	...
	*/
	...
	function save($data,$act=null)
	{
		...
		*/
		...
		$this->db2->select($this->pk_field);
		$this->db2->where($where);
		$query = $this->db2->get($this->schema.'.'.$this->table);
		//
		...
		if($num_rows==0)
		{
			$action = 'New Data';
			// insert new row
			$new = $this->db2->insert($this->schema.'.'.$this->table, $data);		
			$return = $new;
		}elseif($num_rows==1)
		{
			$action = 'Edit Data';
			// update data
			$this->db2->where($where);		
			$update = $this->db2->update($this->schema.'.'.$this->table,$data);		
			$return = $update;
	...
	function saveTo($data)
	{
		...
		if($data[$this->pk_field])
		{
			...
			$update = $this->db2->update($this->table,$data);
			if ($update)
			{
			...
			$insert = $this->db2->insert($this->table, $data);
			//return $insert;
			if ($insert)
			{
	...
	public function delete($where = NULL, $table = FALSE)
	{
		...
		{
			$this->db2->where($this->pk_field, $where);
		}
	
		return $this->db2->delete($table);
				
		//return (int) $this->db2->affected_rows();
	}
	...
	function clean_data($value, $table = FALSE)
	{
		...
		*/
		...
		if ( ! empty($data))
		{
			...
			$fields = $this->db2->list_fields($table);
	...
	{
		$template = array();
		$fields = $this->db2->list_fields($this->table);
	...
	{
		return $this->db2->list_fields($table);
	}
	...
	function gen_xml_structure($schema='')
	{
				...
				<xsd:choice maxOccurs="unbounded">
					<xsd:element name="'.$this->table.'" minOccurs="0" maxOccurs="unbounded">
						<xsd:complexType>
							...
							';
		// get table structure
		$this->db2->select('column_name,data_type,character_maximum_length,numeric_precision,numeric_precision_radix,numeric_scale');
		...
		// die;
		$this->db2->where($where);
		$this->db2->order_by('ordinal_position', 'ASC');
		$query = $this->db2->get('information_schema.columns');
