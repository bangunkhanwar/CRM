...
function point_first_login($membercode,$fidmembercategory)
{
	$_this = & get_Instance();
	...
	$_this->load->model('master/points_model');
	$validate_type = 4;
	$whereValidatePoint = array();
	$whereValidatePoint['MemberCode']= $membercode;		
	$whereValidatePoint['TransType']= $validate_type;
	$memberGetPoint = $_this->points_model->get($whereValidatePoint);
	
	// $save = $this->member_model->save($data);
	//JIKA TIDAK ADA
	if (!$memberGetPoint['MemberCode'])
	{
	
		// INSERT KE TABEL POINT
		// $save = $this->point_model->saveTo($data_point);
		// $this->db->insert('points'.'.'.'Points');
		// $data_point['IdRec']	= 0;		
		$data_point['MemberCode']  = $membercode;		
		$data_point['CreateBy']    = $membercode;		
		$data_point['fidMemberCategory'] = $fidmembercategory;		
		$data_point['Description'] = 'Download Apps';
		$data_point['TransDate']   = date('Y-m-d');
		$data_point['CreateTime']  = date('Y-m-d');
		$data_point['TransType']   = $validate_type; // klik link di email
		$data_point['Debit'] 	   = 50; 
		$data_point['Credit'] 	   = 0;
		
		// $save = $_this->points_model->saveTo($data_point);
		$save_point_new = 'INSERT INTO "points"."Points" ( "TransDate", "CreateBy", "CreateTime", "MemberCode", "fidMemberCategory", "TransType", "Description", "Debit", "Credit", "LastUpdate", "IdRec" ) SELECT
			'.quotedStr(date('Y-m-d')).',
			...
			WHERE
				NOT EXISTS ( SELECT * FROM points."Points" WHERE "MemberCode" = '.quotedStr($membercode).' AND "TransType" = 4 )';
		$_this->db->query($save_point_new);
		// $qry = 'SELECT "sp_InsertMemberPoints"('1902130001','2019','2')
		$tahun = date('Y');
		$bulan = date('n');
		$_this->points_model->posting($membercode,$tahun,$bulan);
...
function validate_input($str='',$params=array())
{
	...
	if(isset($params['required']))
	{
		if($params['required'])
		{
			if($str=='')
			{
				$_this->error('params '.$name.' required'.$str);
			}
		}
	}
		...
		if(isset($params['type']))
		{
			if($params['type'])
			{
				if(($params['type']=='string') || ($params['type']=='varchar') || ($params['type']=='text'))
				{
					if(!is_string($str))
					{
						$_this->error('params '.$name.' type is not '.$params['type']);
					}
				}
				
				if(($params['type']=='number') || ($params['type']=='float') || ($params['type']=='int'))
				{
					if(!is_numeric($str))
					{
						$_this->error('params '.$name.' type is not '.$params['type']);
				...
				if(($params['type']=='date'))
				{
					if(strlen($str)!=8)
					{
						if (!(preg_match("/^[0-9]{4}(0[1-9]|1[0-2])(0[1-9]|[1-2][0-9]|3[0-1])$/",getSQLDate($str)))) {
							$_this->error('params '.$name.' invalid '.$params['type'].' format');
						}
	...
	if(isset($params['maxlength']))
	{
		...
		if($params['maxlength'])
		{
			...
			{
				$_this->error('params '.$name.' exceed maximum length '.$params['maxlength']);
			}
		}
	}
